...
class CuroboController(BaseController):
    ...
    def plan(
        ...
        print("ee_orientation_goal", ee_orientation_goal)
        ik_goal = Pose(
    ...
    def reached_target(self, observations: dict) -> bool:
        ...
        print("current ee position: ", curr_ee_position)
        if np.linalg.norm(
...
while simulation_app.is_running():
    ...
    # print("target position:",my_task.target_position)
    ...
    #########visualize spheres#################
    cu_js = JointState(
    position=my_controller.tensor_args.to_device(sim_js.positions),
    velocity=my_controller.tensor_args.to_device(sim_js.velocities),  # * 0.0,
    acceleration=my_controller.tensor_args.to_device(sim_js.velocities) * 0.0,
    jerk=my_controller.tensor_args.to_device(sim_js.velocities) * 0.0,
    ...
    cu_js = cu_js.get_ordered_joint_state(my_controller.motion_gen.kinematics.joint_names)

    visualize_sphere(my_controller.motion_gen, cu_js)
    #########visualize spheres#################
